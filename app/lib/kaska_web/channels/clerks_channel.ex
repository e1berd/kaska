defmodule KaskaWeb.ClerksChannel do
  @moduledoc """
  `clerks:user:<user_id>` — a user managing their own agents ("clerks") across
  ...
  @impl true
  def join("clerks:user:" <> user_id, _payload, socket) do
    case socket.assigns[:current_user] do
      %{id: ^user_id} = user ->
        {:ok, %{clerks: clerks_view(user.id)}, assign(socket, :owner_id, user.id)}
  ...
  @impl true
  def handle_in("list_clerks", _payload, socket) do
    {:reply, {:ok, %{clerks: clerks_view(socket.assigns.owner_id)}}, socket}
  end
  ...
  def handle_in("create_clerk", payload, socket) do
    ...
    case Agents.create_owned_agent(socket.assigns.owner_id, attrs) do
      {:ok, %{agent: agent, token: token}} ->
        {:reply, {:ok, %{clerk: clerk_view(agent), token: token}}, socket}
  ...
  def handle_in("update_clerk", %{"id" => id} = payload, socket) do
    attrs = take_present(payload, ["display_name", "agent_role", "agent_description"])

    with_clerk(socket, id, fn agent ->
      case Agents.update_agent(agent, attrs) do
        {:ok, updated} ->
          {:reply, {:ok, %{clerk: clerk_view(updated)}}, socket}
  ...
  def handle_in("delete_clerk", %{"id" => id}, socket) do
    with_clerk(socket, id, fn agent ->
  ...
  def handle_in("assign_project", %{"id" => id, "project_id" => project_id}, socket) do
    with_clerk(socket, id, fn agent ->
      if Projects.owner?(Projects.get_project(project_id), socket.assigns.owner_id) do
        ...
        {:reply, {:ok, %{clerk: clerk_view(agent)}}, socket}
      else
  ...
  def handle_in("unassign_project", %{"id" => id, "project_id" => project_id}, socket) do
    with_clerk(socket, id, fn agent ->
      ...
      {:reply, {:ok, %{clerk: clerk_view(reload(agent))}}, socket}
    end)
  end

  def handle_in("request_clerk_avatar_upload", %{"id" => id} = payload, socket) do
    with_clerk(socket, id, fn agent ->
      attrs = %{
  ...
  def handle_in("confirm_clerk_avatar_upload", %{"id" => id, "attachment_id" => att_id}, socket) do
    with_clerk(socket, id, fn agent ->
  ...
  defp with_clerk(socket, id, fun) do
    case Agents.get_owned_agent(socket.assigns.owner_id, id) do
      nil -> {:reply, {:error, %{message: "clerk_not_found"}}, socket}
      agent -> fun.(agent)
    end
  end
  ...
  defp clerks_view(owner_id) do
    owner_id |> Agents.list_for_owner() |> Enum.map(&clerk_view/1)
  end

  defp clerk_view(%User{} = u) do
    %{
      id: u.id,
