defmodule KaskaWeb.SysChannel do
  use Phoenix.Channel
  ...
  def handle_in("confirm_user", %{"id" => id}, socket) do
    if is_admin(socket.assigns[:current_user]) do
      case Accounts.get_user(id) do
        ...
        user ->
          case Kaska.Repo.update(Kaska.Accounts.User.confirm_changeset(user)) do
            {:ok, updated_user} ->
  ...
  def handle_in("delete_user", %{"id" => id}, socket) do
    if is_admin(socket.assigns[:current_user]) do
      case Accounts.get_user(id) do
        ...
        user ->
          case Kaska.Repo.delete(user) do
            {:ok, _} ->
  ...
  def handle_in("change_user_role", %{"id" => id, "role" => role}, socket) do
    if is_admin(socket.assigns[:current_user]) do
      case Accounts.get_user(id) do
        ...
        user ->
          case Kaska.Repo.update(Kaska.Accounts.User.changeset_role(user, %{role: role})) do
            {:ok, updated_user} ->
              {:reply, {:ok, user_view(updated_user, Presence.list(socket))}, socket}
