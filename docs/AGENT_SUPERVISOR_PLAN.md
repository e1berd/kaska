...
Статус: Phase 1 реализована. Есть: `agent_configs`/`agent_runs`, шифрование
ключа (`cloak_ecto`, `Kaska.Vault`), контекст `Kaska.AgentRuntime`, диспатч в
супервизор с токеном на прогон, internal callback, reaper, `agent-supervisor/`,
`agent-runtime/` (Claude Agent SDK + `openai_compatible`), UI: страница
`/agents`, запуск/стоп и живой лог на карточке. Старый flow (клерки с PAT,
`clerk-runner`, `mcp/`, публичный REST, Swagger, `/agent/events`) удалён — агент
существует только как серверный. Дальше: проверка на реальном провайдере,
Phase 1.5–3.
