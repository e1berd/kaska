...
import { useScoutsStore, type Scout, type ScoutProject } from '@/stores/scouts'
import {
  PhRobot,
  PhPlus,
  PhPencilSimple,
...
const scouts = useScoutsStore()
const loading = ref(true)
const error = ref<string | null>(null)
const ownerProjects = ref<ScoutProject[]>([])

const scoutList = computed(() => scouts.list)

const dialog = ref(false)
const editing = ref<Scout | null>(null)
...
onMounted(async () => {
  try {
    await scouts.join()
    ownerProjects.value = await scouts.listProjects()
  } catch {
    error.value = 'Не удалось загрузить скаутов'
  } finally {
    loading.value = false
  }
})

function initial(s: Scout) {
...
async function submit() {
  ...
  try {
    ...
    if (editing.value) {
      await scouts.updateScout(editing.value.id, payload)
      dialog.value = false
    } else {
      const { token } = await scouts.createScout(payload)
...
async function regenerate(s: Scout) {
  if (!confirm(`Новый токен для «${s.display_name}»? Старый перестанет работать.`)) return
  const token = await scouts.regenerateToken(s.id)
  editing.value = s
  form.value = {
    display_name: s.display_name ?? '',
    agent_role: s.agent_role ?? '',
...
async function removeScout(s: Scout) {
  if (!confirm(`Удалить скаута «${s.display_name}»? Он покинет все проекты, токены отзовутся.`))
    return
  await scouts.deleteScout(s.id)
}

function pickAvatar(s: Scout) {
...
async function onAvatarPicked(e: Event) {
  ...
  try {
    await scouts.uploadAvatar(id, file)
  } catch {
...
function availableProjects(s: Scout) {
...
<template>
  <div class="pa-4 pa-sm-6 pa-md-8 mx-auto" style="max-width: 1100px">
    ...
    <p class="md-body-medium text-medium-emphasis mb-6">
      Скаут — твой агент-бот с позывным, характеристикой и токеном для REST/MCP. Назначай скаутов в
    ...
    <v-card
      v-else-if="!scoutList.length"
      variant="outlined"
      class="text-center py-12 md-body-medium text-medium-emphasis"
    >
      Пока нет скаутов. Создай первого — он получит токен для подключения по MCP.
    </v-card>
    ...
    >
      <v-card v-for="s in scoutList" :key="s.id" variant="outlined" rounded="xl" class="pa-4">
        ...
        <div class="d-flex flex-wrap align-center mt-4" style="gap: 8px">
          <v-chip
            ...
            size="small"
            closable
            @click:close="scouts.unassignProject(s.id, p.id)"
          >
            {{ p.name }}
          </v-chip>
          <v-menu v-if="availableProjects(s).length">
            ...
            <v-list density="compact" rounded="lg">
              <v-list-item
                ...
                :key="p.id"
                :title="p.name"
                @click="scouts.assignProject(s.id, p.id)"
              />
            </v-list>
          </v-menu>
          <span
      ...
      <v-card rounded="xl">
        ...
        <v-card-text class="px-6 pt-2 flex flex-col gap-4">
          ...
          <v-textarea
            ...
            label="Описание"
            placeholder="Чем занимается этот скаут"
            variant="filled"
            density="comfortable"
            rows="3"
            auto-grow
