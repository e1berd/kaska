...
const agentMembers = computed(() => members.value.filter((m) => m.is_agent))
...
<template>
  <div class="pa-4 pa-sm-6 pa-md-8 mx-auto" style="max-width: 1200px">
    ...
    <template v-if="agentMembers.length || isOwner">
      <div class="d-flex align-center justify-space-between mt-8 mb-2">
        <h2 class="md-title-medium">Агенты</h2>
        <v-btn v-if="isOwner" variant="tonal" color="primary" :to="{ name: 'agents' }">
          <template #prepend><ph-robot :size="20" weight="bold" /></template>
          Мои агенты
        </v-btn>
      </div>
      <p class="md-body-small text-medium-emphasis mb-3">
        Агенты работают над задачами в контейнере на сервере Kaska. Назначайте их в проект на
        странице «Агенты».
      </p>
      <v-card variant="outlined">
        <v-list v-if="agentMembers.length" class="bg-transparent">
          <v-list-item v-for="a in agentMembers" :key="a.user_id" class="py-2">
            <template #prepend>
              <v-avatar size="32" class="mr-3" color="secondary">
                <img v-if="a.avatar_url" width="32" height="32" :src="a.avatar_url" />
                <span v-else class="text-white">{{ memberLabel(a).slice(0, 1).toUpperCase() }}</span>
              </v-avatar>
            </template>
            <v-list-item-title class="md-body-medium">{{ memberLabel(a) }}</v-list-item-title>
            <v-list-item-subtitle>агент</v-list-item-subtitle>
          </v-list-item>
        </v-list>
        <v-card-text v-else class="text-medium-emphasis md-body-medium">В проекте пока нет агентов.</v-card-text>
      </v-card>
    </template>
